...
def get_event(id:int, db: Session = Depends(get_db) , current_user: CurrentUser = Depends(get_current_user)):
...
def updated_event(id:int, event: UpdateEventSchema, db: Session = Depends(get_db),  current_user: CurrentUser = Depends(get_current_user)):
